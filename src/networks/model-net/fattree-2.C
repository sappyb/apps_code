...
// TensorFlow headers (commented out)
// #include "cppflow/ops.h"
// #include "cppflow/model.h"

using namespace std;

// Conditional inclusion of Cortex library headers
#ifdef ENABLE_CORTEX
...
// Debugging and data collection settings
...
// Enums for routing and rail selection algorithms
enum ROUTING_ALGO
{
  STATIC = 1,
  ADAPTIVE,
...
static char port_file[MAX_NAME_LENGTH];
static char path_file[MAX_NAME_LENGTH];
static char term_sw_file[MAX_NAME_LENGTH];
static char flow_file[MAX_NAME_LENGTH];
static char flow_file_1[MAX_NAME_LENGTH];
static char flow_file_2[MAX_NAME_LENGTH];
static char elephants_out[MAX_NAME_LENGTH];

// Magic numbers for switch and terminal
...
// Flow tables and routing tables
...
unordered_map<string, unordered_map<string, vector<vector<string>>>> inital_flow_path;
unordered_map<string, unordered_map<string, vector<vector<string>>>> inital_flow_path_1;
unordered_map<string, unordered_map<string, vector<vector<string>>>> inital_flow_path_2;
...
// Router buffer load and other routing variables
unordered_map<string, vector<int>> router_buffer_load;
...
void init_fattree_message_list(fattree_message_list *ff, fattree_message *inmsg)
{
  ff->msg = *inmsg;
  ff->event_data = NULL;
  ff->next = NULL;
...
struct fattree_param
{
  ...
  int *switch_radix;     // radix of switches at various levels
  double link_bandwidth; // bandwidth of a wire connecting switches
  double cn_bandwidth;   // bandwidth of the compute node channels connected to switch
...
// FatTree quick hash entry structure
...
// Terminal and switch state structures
...
// ROSS instrumentation support structures
...
// A function to write the port level routing information in a file
void write_port_routing_file()
{
  ofstream wf;
  wf.open(write_flow_file);
  wf.clear();
  for (auto it = sw_ports.begin(); it != sw_ports.end(); ++it)
  {
    for (auto iit = it->second.begin(); iit != it->second.end(); ++iit)
    {
      wf << it->first << " " << iit->first << " ";
...
// A function to write the flow level routing information in a file
void write_flow_routing_file()
{
  ofstream wf;
  wf.open(write_flow_file);
  wf.clear();
  for (auto it = sw_paths.begin(); it != sw_paths.end(); ++it)
  {
    for (auto iit = it->second.begin(); iit != it->second.end(); ++iit)
    {
      wf << it->first << " " << iit->first << " ";
      for (auto iiit = iit->second.begin(); iiit != iit->second.end(); ++iiit)
      {
        for (auto j = iiit->begin(); j != iiit->end(); ++j)
        {
          wf << *j << " ";
        }
        wf << endl;
...
void write_flow_table_to_file(const std::string &filename)
{
  ...
  for (const auto &src_entry : flow_table)
  {
    int src = src_entry.first;
    for (const auto &dest_entry : src_entry.second)
    {
      int dest = dest_entry.first;
      int64_t size = dest_entry.second[0]; // Assuming single size entry for simplicity

      out_file << src << " " << dest << " " << size << "\n";
    }
  }
...
static void fattree_read_config(const char *anno, fattree_param *p)
{
  ...
  // Efficiently handle flow detection and routing table generation
  ...
  if (flow_detect == 4)
  {
    // Read elephants_out file
    elephants_out[0] = '\0';
    rc = configuration_get_value(&config, "PARAMS", "elephants_out", anno, elephants_out, MAX_NAME_LENGTH);

    if (elephants_out[0] != '\0')
    {
      ifstream wf(elephants_out);
      if (!wf)
      {
        tw_error(TW_LOC, "elephants_out file not found");
      }
      else
      {
        string file_src, file_dest, one_line;
        while (getline(wf, one_line))
        {
          istringstream st(one_line);
          st >> file_src >> file_dest;
          elephants_outside.emplace_back("H_" + file_src, "H_" + file_dest);
        }

        // Initial routing table generation
        configuration_get_value_int(&config, "PARAMS", "initial_route", anno, &initial_route);
        if (!g_tw_mynode)
          printf("Initial routing table generation method is %d\n", initial_route);

        if (initial_route == 1)
        {
          flow_file[0] = '\0';
          rc = configuration_get_value(&config, "PARAMS", "flow_file", anno, flow_file, MAX_NAME_LENGTH);
          if (flow_file[0] != '\0')
          {
            ifstream wf(flow_file);
            if (!wf)
            {
            ...
            else
            {
              ...
              while (getline(wf, one_line))
              {
                istringstream st(one_line);
                st >> file_src >> file_dest;
                vector<string> switch_path;
                while (st >> switches)
                {
                  switch_path.push_back(switches);
                }
                inital_flow_path[file_src][file_dest].emplace_back(switch_path);
              }
            }

            // Update flow paths
            for (auto &it : inital_flow_path)
            {
              ...
              for (auto &it2 : it.second)
              {
                ...
                int path_index = -1;
                vector<string> path = it2.second[0];
                const auto &all_paths = sw_paths[src_sw][dest_sw];

                auto path_match = [&](const vector<string> &p1, const vector<string> &p2)
                {
                  return p1.size() == p2.size() && equal(p1.begin(), p1.end(), p2.begin());
                };

                auto it_path = find_if(all_paths.begin(), all_paths.end(), [&](const vector<string> &p)
                                       { return path_match(p, path); });

                if (it_path != all_paths.end())
                {
                  path_index = distance(all_paths.begin(), it_path);
                }
            ...
            // Print flow paths
            for (const auto &it : flow_path)
            {
              string src_term = it.first;
              for (const auto &it2 : it.second)
              {
                string dest_term = it2.first;
                int path_index = it2.second;
                printf("Flow path from %s to %s is %d\n", src_term.c_str(), dest_term.c_str(), path_index);
              }
            }
          }
          if (one_path == 0)
          {
            flow_file_1[0] = '\0';
            rc = configuration_get_value(&config, "PARAMS", "flow_file_1", anno, flow_file_1, MAX_NAME_LENGTH);
            if (flow_file_1[0] != '\0')
            {
              ifstream wf(flow_file_1);
              if (!wf)
              {
                tw_error(TW_LOC, "flow file not found");
              }
              else
              {
                ...
                while (getline(wf, one_line))
                {
                  ...
                  inital_flow_path_1[file_src][file_dest].emplace_back(switch_path);
                }
              }

              // Update flow paths
              for (auto &it : inital_flow_path_1)
              {
                string src_term = it.first;
                for (auto &it2 : it.second)
                {
                  ...
                  if (src_sw != dest_sw)
                  {
                    flow_path_1[src_term][dest_term] = path_index;
                  }
                }
              }

              // Print flow paths
              for (const auto &it : flow_path_1)
              {
                string src_term = it.first;
                for (const auto &it2 : it.second)
                {
            ...
            ///////////
            flow_file_2[0] = '\0';
            rc = configuration_get_value(&config, "PARAMS", "flow_file_2", anno, flow_file_2, MAX_NAME_LENGTH);
            if (flow_file_2[0] != '\0')
            {
              ifstream wf(flow_file_2);
              if (!wf)
              {
                tw_error(TW_LOC, "flow file not found");
              }
              else
              {
                ...
                while (getline(wf, one_line))
                {
                  ...
                  inital_flow_path_2[file_src][file_dest].emplace_back(switch_path);
                }
              }

              // Update flow paths
              for (auto &it : inital_flow_path_2)
              {
                string src_term = it.first;
                for (auto &it2 : it.second)
                {
                  ...
                  if (src_sw != dest_sw)
                  {
                    flow_path_2[src_term][dest_term] = path_index;
                  }
                }
              }

              // Print flow paths
              for (const auto &it : flow_path_2)
              {
                string src_term = it.first;
                for (const auto &it2 : it.second)
                {
...
static void fattree_report_stats()
{
  ...
  if (!g_tw_mynode)
  {
    write_flow_table_to_file("flow_table.txt");
...
{
  // Initialize bookkeeping flags
  bf->c1 = 0;
  bf->c2 = 0;
  bf->c3 = 0;
  tw_stime ts;

  // Extract terminal and switch information
  int dest_term_local_id = codes_mapping_get_lp_relative_id(msg->dest_terminal_id, 0, 0);
  string dest_term = "H_" + to_string(dest_term_local_id);
  ...
  int from_terminal = 0;
  int number_of_switch_per_pod = s->params->Ns;
  int src_pod = int(msg->src_term_id) / (number_of_switch_per_pod * (s->radix / 2));
  int dest_pod = dest_term_local_id / (number_of_switch_per_pod * (s->radix / 2));

  // Check if the flow exists
  int flow_not_found = 1; // Assume flow is not found
  auto src_it = flow_path.find(src_term);
  if (src_it != flow_path.end())
  {
    auto dest_it = src_it->second.find(dest_term);
    if (dest_it != src_it->second.end() && dest_it->second != -1)
    {
      flow_not_found = 0; // Flow is found
    }
  }

  // Select the output port based on routing type
  if (s->params->routing == STATIC || s->params->routing == ADAPTIVE)
  {
    cout << "Routing type not supported" << endl;
    output_port = ft_get_output_port(s, bf, msg, lp, &out_off);
  }
  else if (s->params->routing == SDN)
  {
    // If flow path is not recorded in flow table
    if (flow_not_found == 1)
    {
      cout << "Flow not found : " << src_term << " " << dest_term << endl;
      output_port = ft_get_output_port(s, bf, msg, lp, &out_off);
    ...
    else if (one_path == 1)
    {
      ...
      if (msg->last_hop == TERMINAL)
      {
        ...
        else
        {
          ...
          string src_rr = sw_paths[src_sw][dest_sw][path_index][0];
          string dest_rr = sw_paths[src_sw][dest_sw][path_index][1];
      ...
      else
      {
        if (msg->path_index == -1)
        {
          cout << "Path index not found" << endl;
          output_port = ft_get_output_port(s, bf, msg, lp, &out_off);
        }
        ...
        else if (msg->current_node < sw_paths[src_sw][dest_sw][msg->path_index].size() - 1)
        {
          ...
          string src_rr = sw_paths[src_sw][dest_sw][msg->path_index][msg->current_node];
          string dest_rr = sw_paths[src_sw][dest_sw][msg->path_index][msg->current_node + 1];
          output_port = sw_ports[src_rr][dest_rr][0];
        }
        msg->current_node++;
      }
    ...
    else if (one_path == 0)
    {
      ...
      if (msg->last_hop == TERMINAL)
      {
        ...
        else
        {
          ...
          string src_rr = sw_paths[src_sw][dest_sw][path_index][0];
          string dest_rr = sw_paths[src_sw][dest_sw][path_index][1];
          output_port_1 = sw_ports[src_rr][dest_rr][0];

          int path_index_2 = flow_path_1[src_term][dest_term];
          src_rr = sw_paths[src_sw][dest_sw][path_index_2][0];
          dest_rr = sw_paths[src_sw][dest_sw][path_index_2][1];
          output_port_2 = sw_ports[src_rr][dest_rr][0];

          int path_index_3 = flow_path_2[src_term][dest_term];
          src_rr = sw_paths[src_sw][dest_sw][path_index_3][0];
          dest_rr = sw_paths[src_sw][dest_sw][path_index_3][1];
          output_port_3 = sw_ports[src_rr][dest_rr][0];
          if (random_selection == 0)
          {
            int load_1 = s->vc_occupancy[output_port_1] + s->queued_length[output_port_1];
            int load_2 = s->vc_occupancy[output_port_2] + s->queued_length[output_port_2];
            if (load_2 <= load_1)
            {
              output_port = output_port_2;
              msg->path_index = path_index_2;
              msg->current_node = 1;
            }
            else
            {
          ...
          {
            // Select randomly between the two paths
            if (rand() % 3 == 0)
            {
              output_port = output_port_1;
              msg->path_index = path_index;
            ...
            {
              output_port = output_port_3;
              msg->path_index = path_index_3;
            }
            msg->current_node = 1;
          }
        }
  ...
  /*int output_port_1 = -1;
  int output_port_2 = -1;
  output_port_1 = ft_get_output_port(s, bf, msg, lp, &out_off);
  output_port_2 = output_port;
  ...
  if (msg->last_hop == TERMINAL)
  {
    if (load_2 <= load_1)
    {
      ...
      cout << "SDN Current switch " << current_switch << " Adaptive output port " << output_port_1 << " Adaptive load " << load_1 << " SDN load : " << load_2 << endl;
      output_port = output_port_2;
    }
    else
    {
      if (load_2 > load_1)
        cout << "ADP Current switch " << current_switch << " Adaptive output port " << output_port_1 << " Adaptive load " << load_1 << " SDN load : " << load_2 << endl;
      output_port = output_port_1;
      msg->no_sdn = 1;
    }
  }
  ...
  if (src_pod != dest_pod)
  {
    if (s->switch_level == 1 && msg->no_sdn != 1)
    {
      int output_port_2 = -1;
      output_port_2 = output_port;
      int load_2 = s->vc_occupancy[output_port_2] + s->queued_length[output_port_2];
    ...
    else if (s->switch_level == 1 && msg->no_sdn == 1 && msg->path_index != -1)
    {
      int output_port_2 = -1;

      string src_rr = sw_paths[src_sw][dest_sw][msg->path_index][1];
      string dest_rr = sw_paths[src_sw][dest_sw][msg->path_index][2];
      if (sw_ports[src_rr].find(dest_rr) != sw_ports[src_rr].end())
      {
        output_port_2 = sw_ports[src_rr][dest_rr][0];
        int load_2 = s->vc_occupancy[output_port_2] + s->queued_length[output_port_2];
        if (load_2 <= load_1)
  ...
  // Ensure output port is within valid range
  ...
  int flow_dest = dest_term_local_id;

  // Record the data sent per leaf switch in a subinterval
  if (s->switch_level == 0 && to_terminal == 0 && msg->last_hop == TERMINAL)
  {
    int64_t data_sent = (msg->packet_size < s->params->chunk_size) ? (msg->packet_size % s->params->chunk_size) : s->params->chunk_size;
    if (flow_table.find(flow_src) != flow_table.end())
    {
      if (flow_table[flow_src].find(flow_dest) != flow_table[flow_src].end())
      {
        flow_table[flow_src][flow_dest][0] += data_sent;
      }
      else
      {
        vector<int64_t> temp(1, data_sent);
  ...
  // Create a new message list item for the current chunk
  ...
  // Set virtual channel index and offset for the current chunk
  cur_chunk->msg.vc_index = output_port;
  cur_chunk->msg.vc_off = out_off;
  cur_chunk->msg.my_N_hop++;
  // Check if the virtual channel has enough space to accommodate the chunk
  if (s->vc_occupancy[output_port] + s->params->chunk_size <= max_vc_size)
  {
    bf->c1 = 1;
    switch_credit_send(s, bf, msg, lp, -1);
    append_to_fattree_message_list(s->pending_msgs, s->pending_msgs_tail, output_port, cur_chunk);
    s->vc_occupancy[output_port] += s->params->chunk_size;
    router_buffer_load[current_switch][output_port] += s->params->chunk_size;
    // Schedule the sending of the chunk if not already in send loop
  ...
  {
    // If the virtual channel is full, queue the chunk
    bf->c3 = 1;
  ...
  // Save the output port for potential rollback
