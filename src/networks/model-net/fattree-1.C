...
bool is_routing_static()
{
  int flow_detect;
  int rc;
  const char *anno = nullptr; // Initialize anno as nullptr
  char flow_detect_value[MAX_NAME_LENGTH];

  // Get the flow detection method
  rc = configuration_get_value_int(&config, "PARAMS", "flow_detect", anno, &flow_detect);
  if (rc != 0)
  {
    tw_error(TW_LOC, "Failed to get flow detection method");
  }
  ...
  return (flow_detect == 4); // Assuming 4 indicates static routing
}
...
static void fattree_report_stats()
{
  ...
  if (!g_tw_mynode)
  {
    if (is_routing_static()) // Assuming there is a function to check if routing is static
    {
